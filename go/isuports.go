...
func calculate(ctx context.Context, tenantDB dbOrTx, finishedAt int64, tenantID int64, comp *CompetitionRow) (int64, int64, error) {
	...
	if err := adminDB.SelectContext(
		...
		return 0, 0, fmt.Errorf("error Select visit_history: tenantID=%d, competitionID=%s, %w", tenantID, comp.ID, err)
	}
	billingMap := map[string]string{}
	for _, vh := range vhs {
		// competition.finished_atよりもあとの場合は、終了後に訪問したとみなして大会開催内アクセス済みとみなさない
		if finishedAt < vh.MinCreatedAt {
			continue
		}
	...
	if err != nil {
		return 0, 0, fmt.Errorf("error flockByTenantID: %w", err)
	}
	defer fl.Close()
	...
	if err := tenantDB.SelectContext(
		...
		return 0, 0, fmt.Errorf("error Select count player_score: tenantID=%d, competitionID=%s, %w", tenantID, comp.ID, err)
	}
	...
	// -> 大会が終わっていなければ、playerCount と visitorCount は 0 で良い
	...
	return visitorCount, playerCount, nil
}
...
func competitionFinishHandler(c echo.Context) error {
	...
	visitorCount, playerCount, err := calculate(ctx, tenantDB, now, v.tenantID, comp)
	if err != nil {
		return fmt.Errorf("go: %w")
	}

	if _, err := tenantDB.ExecContext(
		ctx,
		"UPDATE competition SET finished_at = ?, updated_at = ?, visitor_count = ?, player_count = ? WHERE id = ?",
		now, now, visitorCount, playerCount, id,
	); err != nil {
		return fmt.Errorf(
...
func generateRanking(ctx context.Context, tenantDB dbOrTx, tenantID int64, competitionID string) error {
	pss := []PlayerScoreRow{}
	if err := tenantDB.SelectContext(
		ctx,
		&pss,
	...
	for i, rank := range ranks {
		values = append(values, competitionID, int64(i+1), rank.Score, rank.PlayerID, rank.PlayerDisplayName)
		valuesString = append(valuesString, "(?, ?, ?, ?, ?)")
	}

	_, err := tenantDB.ExecContext(ctx, "DELETE FROM ranking WHERE competition_id = ?", competitionID)
	if err != nil {
		return fmt.Errorf("error deleteRanking: %w", err)
	}
	_, err = tenantDB.ExecContext(ctx, "INSERT INTO ranking (competition_id, ranking, score, player_id, player_display_name) VALUES "+strings.Join(valuesString, ", "), values...)
	return err
}
...
func competitionRankingHandler(c echo.Context) error {
	...
	rankings := []RankingRow{}
	if err := tenantDB.SelectContext(ctx, &rankings, "SELECT * FROM ranking WHERE competition_id = ? AND rank >= ? LIMIT 100", competitionID, rankAfter); err != nil {
	...
	if len(rankings) == 0 {
		...
		if err := tenantDB.SelectContext(ctx, &rankings, "SELECT * FROM ranking WHERE competition_id = ? AND rank >= ?", competitionID, rankAfter); err != nil {
			return fmt.Errorf("error select ranking: %s: %w", competitionID, err)
		}
	}
